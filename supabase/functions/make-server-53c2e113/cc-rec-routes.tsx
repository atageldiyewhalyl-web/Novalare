...
- amount: Transaction amount (NEGATIVE for charges/purchases, POSITIVE for payments/credits/refunds)
...
- currency: Currency code (USD, EUR, GBP, CAD, JPY, AUD, CHF, etc.) - CRITICAL: Extract the ACTUAL currency from the statement, do not default to USD
...
- Payment amounts should be POSITIVE (they reduce the credit card balance/liability)
...
- Use NEGATIVE numbers for purchases/charges (they increase the credit card liability)
- Use POSITIVE numbers for credits/refunds (they reduce the liability)
...
app.post('/cc-rec/upload-statement', async (c) => {
  try {
    ...
    if (!companyId || !accountId || !period) {
      return c.json({ error: 'Missing companyId, accountId, or period' }, 400);
    }
    ...
    const storagePath = `${companyId}/${accountId}/${period}/${statementId}_${sanitizedFileName}`;
    ...
    const statementKey = `cc_rec:${companyId}:${accountId}:${period}:statement:${statementId}`;
    ...
    const transactionPromises = transactions.map((txn: any) => {
      ...
      const txnKey = `cc_rec:${companyId}:${accountId}:${period}:txn:${transactionId}`;
...
// Get all statements and transactions for a company/period/account
app.get('/cc-rec/statements', async (c) => {
  try {
    ...
    console.log(`📂 Fetching CC statements for company ${companyId}, account ${accountId}, period ${period}`);

    // Get all statements for this specific account
    const statementPrefix = `cc_rec:${companyId}:${accountId}:${period}:statement:`;
    const statements = await kv.getByPrefix(statementPrefix);

    // Get all transactions for this specific account
    const txnPrefix = `cc_rec:${companyId}:${accountId}:${period}:txn:`;
...
app.delete('/cc-rec/statement/:statementId', async (c) => {
  try {
    ...
    const txnDeletePromises = statementTransactions.map((txn: any) => {
      const txnKey = `cc_rec:${companyId}:${accountId}:${period}:txn:${txn.id}`;
...
app.post('/cc-rec/upload-ledger', async (c) => {
  try {
    ...
    if (!file || !companyId || !accountId || !period) {
      return c.json({ error: 'file, companyId, accountId, and period are required' }, 400);
    }
    ...
    const key = `cc-rec:${companyId}:${accountId}:${period}:ledger`;
    await kv.set(key, {
      fileName: file.name,
      uploadedAt: Date.now(),
...
app.get('/cc-rec/ledger', async (c) => {
  try {
    ...
    if (!companyId || !accountId || !period) {
      return c.json({ error: 'companyId, accountId, and period are required' }, 400);
    }
...
// Save CC ledger data (JSON body - used for QuickBooks sync)
app.post('/cc-rec/ledger', async (c) => {
  try {
    const body = await c.req.json();
    const { companyId, accountId, period, ledger, entries } = body;

    if (!companyId || !accountId || !period || !entries) {
      return c.json({ error: 'companyId, accountId, period, and entries are required' }, 400);
    }

    const key = `cc-rec:${companyId}:${accountId}:${period}:ledger`;
    await kv.set(key, {
      ...(ledger || {}),
      fileName: ledger?.fileName || 'QuickBooks Sync',
      entries,
      uploadedAt: Date.now()
    });

    console.log(`✅ Saved ${entries.length} CC ledger entries for company ${companyId}, account ${accountId}, period ${period}`);
    ...
    console.error('❌ Error saving CC ledger data:', error);
    return c.json({ error: 'Failed to save CC ledger data', details: error.message }, 500);
  }
});
...
- Currency column (optional - currency code like "USD", "EUR", "GBP", "CAD", etc.)
...
- Extract the ACTUAL currency from the document - do not default to USD/EUR
- If you find multiple currencies in the data, note that in the currency_column
...
Return JSON with:
{
  ...
  "default_currency": "USD" (or extract actual: EUR, GBP, CAD, JPY, AUD, CHF, etc.)
}
...
- Currency detection is critical for multi-currency companies`;
  ...
  for (let i = columnMap.header_row + 1; i < lines.length; i++) {
    ...
    // Extract currency
    const currency = columnMap.currency_column !== null
      ? cols[columnMap.currency_column]
      : (columnMap.default_currency || 'USD');
...
app.post('/cc-rec/run-reconciliation', async (c) => {
  try {
    const body = await c.req.json();
    const { companyId, accountId, period } = body;

    if (!companyId || !accountId || !period) {
      return c.json({ error: 'companyId, accountId, and period are required' }, 400);
    }

    console.log(`🔄 Running CC reconciliation for company ${companyId}, account ${accountId}, period ${period}`);
    ...
    // Load ledger entries
    const ledgerKey = `cc-rec:${companyId}:${accountId}:${period}:ledger`;
...
Only return matches with confidence >= 0.60. Return empty array if no good matches.`;
    ...
    const recKey = `cc-rec:${companyId}:${accountId}:${period}:reconciliation`;
...
app.get('/cc-rec/reconciliation', async (c) => {
  try {
    ...
    // If accountId not provided (Month-End Close use case), return 404
    // since we don't know which CC account to check
    if (!accountId) {
...
// GET /cc-rec/status-summary - Lightweight status check for Month-End Checklist
app.get('/cc-rec/status-summary', async (c) => {
  try {
    const companyId = c.req.query('companyId');
    const period = c.req.query('period');
    // AccountId is optional - if simpler implementation needed, we can just check if *any* exists
    // But ideally we want aggregate status. For now, we'll try to find any reconciliation for this period.

    if (!companyId || !period) {
      return c.json({ error: 'companyId and period are required' }, 400);
    }

    // Since we don't have accountId, we need to find ALL CC accounts and check their status
    // OR we can rely on a specific account if provided.
    // The simplified version for checklist: check all keys matching

    const keyPrefix = `cc-rec:${companyId}:`;
    // We need to find keys like cc-rec:{companyId}:{accountId}:{period}:reconciliation
    // This is hard with simple KV get. 
    // ALTERNATIVE: Use the same logic as bank-rec/status-summary which iterates accounts?
    // But we don't have account list here easily without fetching COA.
    // Let's assume for now the user might have provided accountId via query if available, 
    // or we scan common accounts if we can (kv.getByPrefix might be too heavy if we don't filter).

    // Let's stick to the efficient approach:
    // 1. Fetch COA to get CC accounts
    const coaKey = `company_coa_${companyId}`;
    const coaData = await kv.get(coaKey);

    let ccAccounts: any[] = [];
    if (coaData?.accounts) {
      ccAccounts = coaData.accounts.filter((acc: any) =>
        (acc.type === 'Credit Card' || acc.detailType?.includes('CreditCard')) && acc.isActive !== false
      );
    }

    // If no accounts found, return not started
    if (ccAccounts.length === 0) {
      return c.json({ exists: false, totalAccounts: 0 });
    }

    let totalLocked = 0;
    let totalUnmatchedCC = 0;
    let totalUnmatchedLedger = 0;
    let existsCount = 0;

    for (const acc of ccAccounts) {
      const recKey = `cc-rec:${companyId}:${acc.id}:${period}:reconciliation`;
      const rec = await kv.get(recKey);

      if (rec) {
        existsCount++;
        if (rec.locked) totalLocked++;

        if (rec.summary) {
          totalUnmatchedCC += (rec.summary.unmatched_cc_count || 0);
          totalUnmatchedLedger += (rec.summary.unmatched_ledger_count || 0);
        } else {
          totalUnmatchedCC += (rec.unmatched_cc?.length || 0);
          totalUnmatchedLedger += (rec.unmatched_ledger?.length || 0);
        }
      }
    }

    if (existsCount === 0) {
      return c.json({ exists: false, accountCount: ccAccounts.length });
    }

    return c.json({
      exists: true,
      locked: totalLocked === ccAccounts.length, // All must be locked
      lockedCount: totalLocked,
      accountCount: ccAccounts.length,
      unmatchedCCCount: totalUnmatchedCC,
      unmatchedLedgerCount: totalUnmatchedLedger,
      summary: {
        unmatched_cc_count: totalUnmatchedCC,
        unmatched_ledger_count: totalUnmatchedLedger
      }
    });

  } catch (error: any) {
    console.error('❌ Error fetching CC status summary:', error);
...
app.post('/cc-rec/lock-reconciliation', async (c) => {
  try {
    const { company_id, account_id, period } = await c.req.json();

    if (!company_id || !account_id || !period) {
      return c.json({ error: 'company_id, account_id, and period are required' }, 400);
    }

    const key = `cc-rec:${company_id}:${account_id}:${period}:reconciliation`;
...
app.post('/cc-rec/match-items', async (c) => {
  try {
    const { companyId, accountId, period, ccItems, ledgerItems } = await c.req.json();

    if (!companyId || !accountId || !period || !ccItems || !ledgerItems) {
      return c.json({ error: 'companyId, accountId, period, ccItems, and ledgerItems are required' }, 400);
    }

    console.log(`🔗 Manually matching ${ccItems.length} CC items with ${ledgerItems.length} ledger items for ${companyId} - account ${accountId} - ${period}`);
...
app.post('/cc-rec/mark-timing-difference', async (c) => {
  try {
    const body = await c.req.json();
    const { companyId, accountId, period, type, item } = body;

    if (!companyId || !accountId || !period || !type || !item) {
      return c.json({ error: 'companyId, accountId, period, type, and item are required' }, 400);
    }

    console.log(`⏰ CC Rec - Marking as timing difference:`, type, companyId, accountId, period);
...
app.post('/cc-rec/mark-ignored', async (c) => {
  try {
    ...
    console.log(`🙈 CC Rec - Marking as ignored:`, type, companyId, accountId, period);
...
app.post('/cc-rec/request-information', async (c) => {
  try {
    const body = await c.req.json();
    const { companyId, accountId, period, type, item, note } = body;

    if (!companyId || !accountId || !period || !type || !item || !note) {
      return c.json({ error: 'companyId, accountId, period, type, item, and note are required' }, 400);
    }

    console.log(`📝 CC Rec - Adding to follow-up:`, type, companyId, accountId, period);
