// Server with Google Document AI support - Updated with secrets
...
import teamRoutes from "./team-routes.tsx";
...
import documentRoutes from "./document-routes.tsx";
import emailSettingsRoutes from "./email-settings-routes.tsx";
import { runAWSDiagnostics } from "./aws-diagnostic.tsx";

const app = new Hono();

// Global error handler to catch any unhandled errors and prevent crashes
app.onError((err, c) => {
...
// AWS Diagnostics endpoint - CRITICAL for debugging Textract issues
app.get("/make-server-53c2e113/aws-diagnostics", async (c) => {
  try {
    console.log('🔍 Starting AWS diagnostics...');
    const result = await runAWSDiagnostics();
    console.log('📊 Diagnostics complete:', result);

    // Return structured result for UI display
    return c.json({
      status: result?.success ? 'success' : 'error',
      result: result || { success: false, error: 'NO_RESULT' },
      message: result?.success
        ? 'AWS Textract is activated and working!'
        : 'Textract activation issue detected - check server logs for detailed troubleshooting'
    });
  } catch (error: any) {
    ...
    return c.json({
      ...
      details: 'Check server logs for full diagnostic output'
    }, 500);
  }
});

// Test Python Extraction API endpoint
app.get("/make-server-53c2e113/test-python-api", async (c) => {
  ...
  try {
    console.log('🐍 Testing Python API at:', pythonApiUrl);
    ...
    if (response.ok) {
      ...
      console.log('✅ Python API is online:', data);
      return c.json({
        status: 'online',
        message: `Python API is running (${data.status || 'healthy'})`,
        api_url: pythonApiUrl,
        response: data
      });
    } else {
      console.error('❌ Python API returned error:', response.status);
      return c.json({
        status: 'offline',
...
// Debug endpoint to check Google Document AI credentials (PUBLIC - no auth required)
app.get("/make-server-53c2e113/debug/google-credentials", (c) => {
  const projectId = Deno.env.get('GOOGLE_PROJECT_ID');
  const processorId = Deno.env.get('GOOGLE_PROCESSOR_ID');
  const location = Deno.env.get('GOOGLE_PROCESSOR_LOCATION');
  const credentialsJson = Deno.env.get('GOOGLE_APPLICATION_CREDENTIALS_JSON');

  // Add CORS headers to make it accessible from browser
  c.header('Access-Control-Allow-Origin', '*');
  c.header('Access-Control-Allow-Methods', 'GET, OPTIONS');
  c.header('Access-Control-Allow-Headers', '*');

  return c.json({
    GOOGLE_PROJECT_ID: projectId ? `SET (${projectId.substring(0, 20)}...)` : 'NOT SET ❌',
    GOOGLE_PROCESSOR_ID: processorId ? `SET (${processorId.substring(0, 20)}...)` : 'NOT SET ❌',
    GOOGLE_PROCESSOR_LOCATION: location ? `SET (${location})` : 'NOT SET ❌',
    GOOGLE_APPLICATION_CREDENTIALS_JSON: credentialsJson ? `SET (${credentialsJson.length} chars)` : 'NOT SET ❌',
  });
});

// Mount all API routes
app.route('/make-server-53c2e113', routes);
app.route('/make-server-53c2e113', monthEndCloseRoutes);
app.route('/make-server-53c2e113', authRoutes);
app.route('/make-server-53c2e113', teamRoutes);
app.route('/make-server-53c2e113', bankRecRoutes);
app.route('/make-server-53c2e113', apRecRoutes);
app.route('/make-server-53c2e113', arRecRoutes);
app.route('/make-server-53c2e113', journalEntriesRoutes);
app.route('/make-server-53c2e113', receiptRoutes);
app.route('/make-server-53c2e113', ccRecRoutes);
app.route('/make-server-53c2e113', documentRoutes);
app.route('/make-server-53c2e113', trialBalanceRoutes);
app.route('/make-server-53c2e113', accountingIntegrationRoutes);
app.route('/make-server-53c2e113', emailSettingsRoutes);
...
function performSumBasedMatching(
  ...
  // Sum-based matching temporarily disabled - return unmatched
  return {
    groupedMatches: [],
    unmatchedBank: bankTransactions,
    unmatchedLedger: ledgerEntries
  };
}
...
// NOTE: 10-K Analysis endpoint is defined later in the file (around line 582)
// Duplicate removed to prevent server crash from duplicate route definitions

// Orphaned OAuth code below - will be cleaned up later
// For now, just making it syntactically valid by adding missing context
/*
      headers: {
    ...
    if (!tokenResponse.ok) {
      return c.html(`<html><body><script>window.opener?.postMessage({type:'qbo-oauth-error',error:'Token exchange failed'},'*');window.close();</script></body></html>`);
    }
    ...
    const encryptToken = async (token: string): Promise<string> => {
      ...
      const key = await crypto.subtle.importKey('raw', hashBuffer, { name: 'AES-GCM', length: 256 }, false, ['encrypt']);
    ...
    existingConnections.push({ id: connectionId, type: 'quickbooks', realm_id: realmId, company_name: companyName, connected_at: connection.created_at });
    await kv.set(`firm:${firmId}:accounting_connections`, existingConnections);
    
    await kv.set(`sync_state:${connectionId}`, { connection_id: connectionId, accounts_synced: false, company_info_synced: false, vendors_synced: false, customers_synced: false });
    
    console.log(`✅ QB connected: ${connectionId}`);
    
    return c.html(`<html><body><h2>✅ Connected to ${companyName}!</h2><script>window.opener?.postMessage({type:'qbo-oauth-success',connection:{id:'${connectionId}',company_name:'${companyName.replace(/'/g, "\\'")}',realm_id:'${realmId}',type:'quickbooks'}},'*');setTimeout(()=>window.close(),2000);</script></body></html>`);
  } catch (error: any) {
    console.error('❌ Callback error:', error);
    return c.html(`<html><body><script>window.opener?.postMessage({type:'qbo-oauth-error',error:'${error.message.replace(/'/g, "\\'")}'}, '*');window.close();</script></body></html>`);
  }
});
*/

// Continue with real route definitions...
// (AWS diagnostics and other routes are already defined above at lines 60-150)

// NOTE: All API routes already mounted at lines 151-162

/* ============================================================================
   DUPLICATE ROUTE HANDLERS COMMENTED OUT
   Real implementations exist earlier in the file (lines 60-150)
   ============================================================================

// Skip all duplicate orphaned route code below
    console.log('📊 Diagnostics complete:', result);
...
// ============================================================================
// END OF COMMENTED DUPLICATE SECTION
// Helper functions and routes are defined above at lines 60-374
// ============================================================================
  ...
  try {
    ...
    if (fileType === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
      ...
      try {
        ...
        const chunkSize = 1024; // Reduced from 8192 to prevent stack overflow
        let base64 = '';
  ...
  try {
    ...
    if (vendorFile.type === 'application/pdf' || vendorFile.name.endsWith('.pdf')) {
      ...
      for (let i = 0; i < uint8Array.length; i += chunkSize) {
        ...
        // Use Array.from() to prevent call stack size errors
...
app.post("/make-server-53c2e113/export-ap-reconciliation", async (c) => {
  try {
    ...
    console.log('📝 Reconciliation data:', JSON.stringify(result, null, 2));
    ...
    const summaryData = [
      ...
      ['Total Vendor Amount', `€${(result.summary?.total_vendor_amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`],
      ['Total AP Ledger Entries', result.summary?.total_ap_entries || 0],
      ['Total AP Amount', `€${(result.summary?.total_ap_amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`],
      [],
    ...
    if (result.matched_pairs && result.matched_pairs.length > 0) {
      const matchedData = [
        ...
        ...result.matched_pairs.map((m: any) => [
          ...
          `€${Math.abs(m.vendor_transaction?.amount || 0).toFixed(2)}`,
          m.match_type || 'N/A',
    ...
    if (result.unmatched_vendor && result.unmatched_vendor.length > 0) {
      const unmatchedVendorData = [
        ...
        ...result.unmatched_vendor.map((m: any) => [
          ...
          `€${Math.abs(m.transaction?.amount || 0).toFixed(2)}`,
    ...
    if (result.unmatched_ap && result.unmatched_ap.length > 0) {
      const unmatchedAPData = [
        ...
        ...result.unmatched_ap.map((i: any) => [
          ...
          `€${Math.abs(i.entry?.amount || 0).toFixed(2)}`,
          i.entry?.account || 'N/A',
...
app.post("/make-server-53c2e113/export-vendor-statements", async (c) => {
  try {
    console.log('📊 Starting vendor statements Excel export...');
    ...
    console.log('📝 Creating Excel workbook with', transactions.length, 'transactions...');
    ...
    const summaryData = [
      ...
      ['Total Amount', `€${transactions.reduce((sum, t) => sum + (t.amount || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`],
    ];
    ...
    console.error('❌ Vendor statements Excel export error:', error);
    return c.json({
      error: 'Excel export failed',
      details: error.message
    }, 500);
...
// ============================================================================
// QUICKBOOKS TOKEN UTILITIES
// ============================================================================

/**
 * Get encryption key derived from service role key
 */
 ...
 */
async function refreshQBOToken(connection: any): Promise<any> {
...
// Get Company by ID (with automatic connection ID fixing)
app.get("/make-server-53c2e113/companies/:companyId", async (c) => {
  try {
    const companyId = c.req.param('companyId');
    ...
    console.log(`📊 Fetching company ${companyId} for firm: ${firmId}`);
    ...
    if (!company) {
      console.error(`❌ Company ${companyId} not found for firm ${firmId}`);
      return c.json({ error: 'Company not found' }, 404);
    }

    // If company has a qbo_connection_id, verify it exists and fix if stale
    if (company.qbo_connection_id && company.realm_id) {
      const connection = await kv.get(`qbo_connection:${company.qbo_connection_id}`);

      if (!connection) {
        // Connection ID is stale, try to find by realm_id
        console.log(`🔍 Stale connection ID detected, looking up by realm_id: ${company.realm_id}`);
        const correctConnectionId = await kv.get(`firm:${company.firm_id}:qbo_connection:${company.realm_id}`);

        if (correctConnectionId) {
          console.log(`✅ Found correct connection ID: ${correctConnectionId}, updating company record`);
          company.qbo_connection_id = correctConnectionId;
          await kv.set(`firm:${firmId}:company:${companyId}`, company);
        } else {
          console.log(`⚠️ No valid connection found for realm ${company.realm_id}`);
          // Clear the stale connection ID
          company.qbo_connection_id = undefined;
          await kv.set(`firm:${firmId}:company:${companyId}`, company);
        }
      }
    }
...
// Sync QuickBooks Accounts for a Company
app.post("/make-server-53c2e113/companies/:companyId/sync-qbo-accounts", async (c) => {
  try {
    const companyId = c.req.param('companyId');
    console.log('🔄 Syncing QuickBooks accounts for company:', companyId);

    // Get company to find its QBO connection (use colon format)
    ...
    if (!company.qbo_connection_id) {
      console.error(`❌ Company ${companyId} has no QuickBooks connection`);
      return c.json({ error: 'No QuickBooks connection found for this company' }, 400);
    }

    console.log(`📋 Found company "${company.name}" with QBO connection: ${company.qbo_connection_id}`);

    // Get the QBO connection
    let connection = await kv.get(`qbo_connection:${company.qbo_connection_id}`);

    if (!connection) {
      console.error(`❌ QuickBooks connection not found: ${company.qbo_connection_id}`);

      // Debug: Try to find all connections for this company's firm
      console.log(`🔍 Attempting to find connections for firm ${company.firm_id}...`);
      const firmConnections = await kv.get(`firm:${company.firm_id}:accounting_connections`) || [];
      console.log(`📋 Found ${firmConnections.length} total firm connections:`, JSON.stringify(firmConnections, null, 2));

      // Try to find connection by realm_id as well
      if (company.realm_id) {
        console.log(`🔍 Trying to find connection by realm_id: ${company.realm_id}`);
        const connectionIdByRealm = await kv.get(`firm:${company.firm_id}:qbo_connection:${company.realm_id}`);
        console.log(`📋 Connection ID by realm_id:`, connectionIdByRealm);

        if (connectionIdByRealm) {
          connection = await kv.get(`qbo_connection:${connectionIdByRealm}`);
          if (connection) {
            console.log(`✅ Found connection via realm_id lookup! Updating company record...`);

            // Update company with correct connection ID
            company.qbo_connection_id = connectionIdByRealm;
            await kv.set(`company:${companyId}`, company);
            console.log(`✅ Updated company ${companyId} with correct connection ID: ${connectionIdByRealm}`);
          }
        }
      }
      ...
      if (!connection) {
        return c.json({
          error: 'QuickBooks connection not found. Please reconnect your QuickBooks account from the Integrations tab.',
          debug: {
            connectionId: company.qbo_connection_id,
            firmId: company.firm_id,
            realmId: company.realm_id,
            totalFirmConnections: firmConnections.length
          }
        }, 404);
      }
    }

    console.log(`✅ Found QBO connection for realm ${connection.realm_id}`);

    // Refresh tokens if needed (using the shared utility function)
    ...
    const qboResponse = await fetch(
      `https://sandbox-quickbooks.api.intuit.com/v3/company/${connection.realm_id}/query?query=SELECT * FROM Account MAXRESULTS 1000&minorversion=65`,
      {
        headers: {
    ...
    if (!qboResponse.ok) {
      ...
      return c.json({ error: 'Failed to fetch accounts from QuickBooks' }, 500);
    }
    ...
    // Save to company COA
    await kv.set(`company_coa_${companyId}`, { accounts: coaAccounts });

    console.log(`✅ Synced ${coaAccounts.length} accounts to company ${companyId} COA`);
    ...
    console.error('❌ Failed to sync QuickBooks accounts:', error);
    return c.json({
...
app.route('/', emailSettingsRoutes);

console.log('🚀🚀🚀 NOVALARE SERVER STARTED! All routes mounted successfully.');
console.log('📍 Bank rec route available at: /bank-rec/upload-bank-statement-stream');
...
app.post("/make-server-53c2e113/api/receipts/upload", async (c) => {
  try {
    ...
    console.log('📎 Processing receipt:', file.name, 'for company:', companyId);
    ...
    console.log('📷 File MIME type:', mimeType, '| Size:', (arrayBuffer.byteLength / 1024 / 1024).toFixed(2), 'MB');

    const imageUrl = `data:${mimeType};base64,${base64Data}`;

    // Extract data using OpenAI Vision
    console.log('🤖 Calling OpenAI to extract receipt data...');
    const openaiKey = Deno.env.get('OPENAI_API_KEY');
    if (!openaiKey) {
      throw new Error('OpenAI API key not configured');
    }
...
Return ONLY a JSON object with these fields. Be accurate with numbers and dates.`
    ...
    try {
      ...
      console.error('⚠️ Bucket creation error (may already exist):', bucketError);
    }
    ...
    console.log('✅ Receipt file uploaded to storage:', filePath);
    ...
    // Save to KV store
    console.log('💾 Saving receipt to KV with filePath:', filePath);
    ...
    console.log('🔍 Verification - Retrieved receipt:', JSON.stringify(savedReceipt, null, 2));
    if (!savedReceipt?.filePath) {
      console.error('⚠️ WARNING: Receipt was saved but filePath is missing after retrieval!');
    }
...
app.delete("/make-server-53c2e113/api/receipts/:companyId/:receiptId", async (c) => {
  try {
    ...
    if (receipt && receipt.filePath) {
      ...
      console.log('🗑️ Deleted file from storage:', receipt.filePath);
    }
...
app.get("/make-server-53c2e113/api/receipts/:companyId/:receiptId/view", async (c) => {
  try {
    ...
    console.log(`👁️ Getting view URL for receipt ${receiptId} in company ${companyId}`);
    ...
    if (!receipt) {
      console.error('❌ Receipt not found in KV store');
      return c.json({ error: 'Receipt not found' }, 404);
    }
    ...
    if (receipt.filePath) {
      ...
      console.log('📦 Getting signed URL from bucket:', bucketName, 'path:', receipt.filePath);
      ...
      console.log('✅ Created signed URL for receipt');
      return c.json({
      ...
      console.log('✅ Using existing imageUrl for receipt');
      return c.json({
        url: receipt.imageUrl,
        fileName: receipt.fileName
      });
    } else {
      console.error('❌ Receipt has no filePath or imageUrl - it may have been uploaded before storage was enabled');
      return c.json({ error: 'Receipt file not available. Please re-upload this receipt.' }, 404);
    }

  } catch (error) {
    console.error('❌ Error getting receipt view URL:', error);
    return c.json({
...
app.post("/make-server-53c2e113/api/receipts/export/xlsx", async (c) => {
  try {
    ...
    console.log(`📊 Exporting ${receipts.length} receipts to XLSX`);
...
// Global uncaught error handlers to prevent crashes
