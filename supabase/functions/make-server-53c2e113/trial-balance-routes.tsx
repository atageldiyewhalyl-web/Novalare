...
// GET /trial-balance/status-summary - Lightweight status check for Month-End Checklist
app.get('/make-server-53c2e113/trial-balance/status-summary', async (c) => {
  try {
    const companyId = c.req.query('companyId');
    const period = c.req.query('period');
    ...
    // Return essential status info
    return c.json({
      exists: true,
      uploadedAt: result.uploadedAt,
      isBalanced: result.isBalanced,
      canClose: result.canClose,
      errorCount: result.structuralErrors?.length || 0,
      warningCount: result.analyticalWarnings?.length || 0,
      summary: result.summary
    });
  } catch (error: any) {
    console.error('❌ Error fetching trial balance status summary:', error);
    return c.json({ error: `Failed to fetch status summary: ${error.message}` }, 500);
  }
});
