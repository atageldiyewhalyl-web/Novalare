...
app.get('/month-close/status', async (c) => {
  ...
  try {
    ...
    console.log(`🔒 Checking lock status for ${companyId}/${period}...`);
    const kvStartTime = Date.now();
    const lockStatus = await kv.get(`month-close-lock:${companyId}:${period}`);
    const kvEndTime = Date.now();
    console.log(`✅ KV lookup took ${kvEndTime - kvStartTime}ms`);
    
    if (!lockStatus) {
      const totalTime = Date.now() - startTime;
      console.log(`✅ Period is unlocked (total: ${totalTime}ms)`);
      return c.json({ isLocked: false });
    }
    
    const totalTime = Date.now() - startTime;
    console.log(`✅ Period is locked (total: ${totalTime}ms)`);
    return c.json(lockStatus);
  } catch (error) {
    const totalTime = Date.now() - startTime;
    console.error(`❌ Error checking lock status (${totalTime}ms):`, error);
