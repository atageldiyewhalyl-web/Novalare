...
/**
 * Helper: Detect currency from amount string (symbols/codes)
 */
function detectCurrencyFromAmountString(amountStr: string): string | null {
  if (!amountStr) return null;
  const s = amountStr.trim().toUpperCase();
  ...
  if (s.includes('C¥') || s.includes('CN¥')) return 'CNY';
  if (s.includes('CHF')) return 'CHF'; // Often prefix

  // Codes (suffix or prefix)
  ...
  // Ambiguous '$' - assume USD if no other context, but be careful
  // Only return if explicitly '$' and not just a number
  if (s.includes('$') && !s.includes('CA') && !s.includes('A') && !s.includes('NZ') && !s.includes('HK') && !s.includes('SG')) {
    return 'USD';
  }
...
Return JSON with:
{
  ...
  for (let i = columnMap.header_row + 1; i < lines.length; i++) {
    ...
    // IMPROVED: Check individual amount string for currency symbol if not explicit column
    const amountValRaw = cols[columnMap.amount_column];
    const detectedCurrency = detectCurrencyFromAmountString(amountValRaw || '');
...
Return JSON with:
{
  ...
  // Get sheet range to calculate absolute cell addresses
  ...
  for (let i = columnMap.header_row + 1; i < jsonData.length; i++) {
    ...
    // Get currency for this transaction (per-row currency or default)
    // IMPROVED: Check individual amount string for currency symbol if not explicit column
    // For Excel: Check the Formatted Text (.w) and Format Code (.z)
    let detectedCurrency: string | null = null;

    // 1. Check raw value if it's a string
    const amountValRaw = String(row[columnMap.amount_column] || '');
    detectedCurrency = detectCurrencyFromAmountString(amountValRaw);

    // 2. If not found, check Excel Cell Formatting (CRITICAL for CA$, ¥, etc.)
    if (!detectedCurrency) {
      try {
        const absRow = range.s.r + i;
        const absCol = range.s.c + columnMap.amount_column;
        const cellAddress = XLSX.utils.encode_cell({ r: absRow, c: absCol });

        const cell = firstSheet[cellAddress];
        if (cell) {
          if (cell.w) detectedCurrency = detectCurrencyFromAmountString(cell.w);
          if (!detectedCurrency && cell.z) detectedCurrency = detectCurrencyFromAmountString(cell.z);
        }
      } catch (err) {
        console.warn('⚠️ Error checking Excel cell formatting:', err);
      }
    }
