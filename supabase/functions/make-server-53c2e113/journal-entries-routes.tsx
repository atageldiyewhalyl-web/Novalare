...
app.post('/journal-entries/approve-suggestion', async (c) => {
  try {
    ...
    const { companyId, period, type, item, source, accountId, sourceAccountName } = body;

    if (!companyId || !period || !type || !item) {
      return c.json({ error: 'companyId, period, type, and item are required' }, 400);
    }

    console.log(`📝 Approving ${type} transaction for JE generation:`, companyId, period, 'source:', source, 'accountId:', accountId, 'sourceAccountName:', sourceAccountName);
    ...
    const suggestedJE = {
      ...
      sourceItem: item,
      source_account_name: sourceAccountName,  // Store the source account name for AI
    ...
    if (source === 'cc-rec') {
      ...
      // Use accountId if available for bank recs
      recKey = accountId
    ...
    // Only attempt to remove from unmatched if reconciliation data exists
    if (reconciliationData) {
      ...
      console.log(`⚠️ No reconciliation data found at key: ${recKey}. Skipping removal from unmatched list.`);
    }

    // ALWAYS save to resolved bucket (moved OUTSIDE the conditional)
    ...
    if (isCCRec) {
      ...
      // Use accountId for bank recs to match data loading key structure
    ...
    console.log(`✅ Added item to resolved bucket at key: ${resolvedKey}`);
...
app.post('/journal-entries/bulk-generate', async (c) => {
  try {
    ...
    const transactionsForAI = transactions.map((suggestion: any) => {
      ...
      // Get source account name from multiple possible locations
      const sourceAccountName = suggestion.source_account_name ||
        suggestion.sourceItem?.source_account_name ||
        suggestion.sourceAccountName;
      ...
      return {
        ...
        direction: transaction.amount < 0 ? 'MONEY OUT (Expense/Payment)' : 'MONEY IN (Income/Deposit)',
        sourceAccount: sourceAccountName,  // Now correctly populated
    ...
    // Call OpenAI API for bulk JE generation with enhanced prompt
    const prompt = `You are an expert accountant with deep knowledge of common merchants and vendors. I will provide you with a list of bank transactions and a chart of accounts. For each transaction, generate a balanced journal entry with appropriate debit and credit accounts.

MERCHANT CLASSIFICATION GUIDE (use these to identify the correct expense category):
- LYFT, UBER, TAXI, CAB, RIDESHARE → Transportation/Travel Expense
- AMAZON, STAPLES, OFFICE DEPOT, OFFICE MAX → Office Supplies OR Inventory (depending on business)
- SLACK, ZOOM, MICROSOFT, ADOBE, GOOGLE, DROPBOX → Software & Subscriptions
- DOORDASH, GRUBHUB, UBER EATS, SEAMLESS, RESTAURANT → Meals & Entertainment
- DELTA, UNITED, AMERICAN AIRLINES, SOUTHWEST, JETBLUE → Travel Expense (Airfare)
- HILTON, MARRIOTT, AIRBNB, HOTEL, HYATT, IHG → Travel Expense (Lodging)
- AT&T, VERIZON, T-MOBILE, COMCAST, INTERNET → Telephone/Utilities
- GEICO, STATE FARM, PROGRESSIVE, INSURANCE → Insurance Expense
- PAYROLL, ADP, GUSTO, WAGES → Payroll Expense
- BANK FEE, WIRE FEE, ATM FEE, OVERDRAFT, SERVICE CHARGE, MONTHLY FEE → Bank Charges
- ACH DEPOSIT, TRANSFER, CUSTOMER PAYMENT → Usually income/receivables
...
CRITICAL RULES:
1. Each transaction has a 'direction' field:
   - If direction is 'MONEY OUT (Expense/Payment)': This is an EXPENSE
     → DEBIT an expense account (match merchant to expense category above)
     → CREDIT the 'sourceAccount' (the bank account where money left from)
   
   - If direction is 'MONEY IN (Income/Deposit)': This is INCOME
     → DEBIT the 'sourceAccount' (the bank account where money arrived)
     → CREDIT an income or receivables account

2. The 'sourceAccount' is the bank account name - ALWAYS use this exact name for the bank side of the entry.

3. ONLY use "Bank Charges" for actual bank fees (ATM fee, wire fee, service charge, overdraft).
   DO NOT use Bank Charges for business expenses like LYFT, AMAZON, UBER, etc.

4. Match the description/merchant to the most appropriate expense category from the guide above.
