  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMsg = data.details ? `${data.error}: ${data.details}` : data.error || `Server error ${response.status}`;
      ...
      const errorMessage = err.message || 'Failed to process reconciliation. Please try again.';
            ...
            >
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 sm:p-8">
                <h3 className="text-2xl text-white mb-6 flex items-center gap-2">
                  ...
                  Matched Transactions ({result.matched_pairs.length})
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {(showAllMatches ? result.matched_pairs : result.matched_pairs.slice(0, 10)).map((pair, idx) => (
