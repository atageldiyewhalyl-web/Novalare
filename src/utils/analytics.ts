...
export interface TrackAuthEventParams {
...
export async function trackAuthEvent(params: TrackAuthEventParams): Promise<void> {
  try {
    const response = await fetch(
      `https://${projectId}.supabase.co/functions/v1/make-server-53c2e113/analytics/track-auth`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
