...
app.get('/accounting/qbo/callback', async (c) => {
  try {
    ...
    // Skip company info fetch during OAuth callback to avoid 403 errors
    // QuickBooks authorization can take 30-60 seconds to fully propagate
    // User can click "Refresh Name" button after connection completes
    console.log(`🏢 Skipping company info fetch during OAuth callback for realm ${realmId}`);
    console.log(`   Note: QuickBooks authorization needs 30-60 seconds to propagate`);
    console.log(`   User can refresh company name after connection using "Refresh Name" button`);
