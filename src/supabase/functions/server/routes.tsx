...
async function initializeBucket() {
  try {
    ...
    if (!bucketExists) {
      ...
      if (error) {
        // Ignore "already exists" errors (race condition handling)
        if (error.message?.includes('already exists') || error.statusCode === '409' || error.status === 400) {
          console.log('✅ Storage bucket already exists (detected during creation)');
        } else {
