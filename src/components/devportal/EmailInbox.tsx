...
export function EmailInbox({ emails, companyId, companyEmail, onSendEmail }: EmailInboxProps) {
  ...
  const forwardingEmail = companyEmail || `invoices+${companyId}@novalare.app`;
        ...
        <Dialog open={!!selectedEmail} onOpenChange={() => setSelectedEmail(null)}>
          <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4 mt-4">
              ...
              {selectedEmail.extractedInvoices > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <CheckCircle2 className="size-5 text-green-700 flex-shrink-0 mt-0.5" />
                    <div className="flex-1">
                      <p className="text-green-900 mb-1">
                        Successfully extracted {selectedEmail.extractedInvoices} invoice
                        {selectedEmail.extractedInvoices !== 1 ? 's' : ''} from this email
                      </p>
                      <p className="text-sm text-green-700">
                        ✓ Invoice metadata extracted with AI<br />
                        ✓ Original files stored securely<br />
                        ✓ Ready for review in Invoice Extraction workflow
                      </p>
                    </div>
                  </div>
                </div>
